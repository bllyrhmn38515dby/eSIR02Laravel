    ...
    <div class="row justify-content-center">
        <div class="col-lg-6">
            <div class="d-flex align-items-center mb-4 animate__animated animate__fadeIn">
                ...
                <div>
                    <h2 class="fw-bold mb-0">Registrasi Armada</h2>
                    <p class="text-muted mb-0">Tambahkan unit ambulans baru ke dalam sistem fleet.</p>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-lg rounded-4 overflow-hidden animate__animated animate__zoomIn">
                <div class="card-header bg-primary py-4 px-4 border-0" style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%);">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="mb-0 text-white fw-bold">Detail Kendaraan</h5>
                    </div>
                </div>
                    ...
                    <form action="{{ route('ambulances.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <select name="faskes_id" class="form-select bg-light border-start-0 py-3 @error('faskes_id') is-invalid @enderror" required>
                                    <option value="" selected disabled>Pilih Faskes Pemilik Armada...</option>
                                    @foreach($faskes as $f)
                                        <option value="{{ $f->id }}" {{ old('faskes_id') == $f->id ? 'selected' : '' }}>{{ $f->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('faskes_id')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="police_number" class="form-control bg-light border-start-0 py-3 fw-bold ls-1 @error('police_number') is-invalid @enderror" placeholder="B 1234 ABC" required value="{{ old('police_number') }}" style="text-transform: uppercase;">
                            </div>
                            @error('police_number')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                            <small class="text-muted mt-2 d-block small">Pastikan plat nomor unik dan tidak mengandung spasi berlebih.</small>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="vehicle_type" class="form-control bg-light border-start-0 py-3 @error('vehicle_type') is-invalid @enderror" placeholder="Contoh: Toyota Hiace / Daihatsu GranMax" required value="{{ old('vehicle_type') }}">
                            </div>
                        </div>

                        <div class="mb-5">
                            <label class="form-label fw-bold text-dark mb-1">Initial Status</label>
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <input type="radio" class="btn-check" name="status" id="status_standby" value="standby" checked>
                                ...
                                <div class="col-md-4">
                                    <input type="radio" class="btn-check" name="status" id="status_ontrip" value="on_trip">
                                ...
                                <div class="col-md-4">
                                    <input type="radio" class="btn-check" name="status" id="status_maintenance" value="maintenance">
                        ...
                        <div class="pt-3">
                            <button type="submit" class="btn btn-primary w-100 py-3 rounded-pill fw-bold shadow-lg ls-1" style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); border: none;">
                                <i class="bi bi-cloud-arrow-up-fill me-2"></i>Simpan Armada Baru
                            </button>
                        </div>
                    </form>
                </div>
